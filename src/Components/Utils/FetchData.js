...
export function fetchData(lang) {
    ...
    return client.getEntries({ 'locale': lang }).then((response) => {
        ...
        let navi, main, player, footer, bio, contacts, projects = [], mConcert, mFilm, news = {'locale': lang, articles:[]};
        response.items.forEach(entry => {
            ...
            if (entry.sys.contentType.sys.id === CLASS_ARTICLES) {
                news.articles.push({...entry.fields, ...{id:entry.sys.id}, ...{createdAt:entry.sys.createdAt}})
            }
            if (entry.sys.id === CLASS_MUSIC_CONCERT) {
                mConcert = entry.fields
            }
            if (entry.sys.id === CLASS_MUSIC_FILM) {
                // const listVideo = ['local','AQbhU0hAnA0','Mi9ANgb8g90','X--dRCrE1x4','Sc4hyaKshIY']
                mFilm = entry.fields
            }
        })
        const music = [];
        response.includes.Asset.forEach(entry => {
            ...
            if (entry.fields.file.contentType.startsWith('audio'))
                music.push(entry.fields)
        })
        
        return {navi, main, player, footer, bio, contacts, projects, mConcert, mFilm, news, lang:lang, music}
    })
}
